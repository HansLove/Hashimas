...
contract HashLock {
    ...
    modifier hashlockMatches(bytes32 _contractId, string memory _x) {
        require(
            contracts[_contractId].hashlock == calcular(_x),
    ...
    function newContract(
        ...
        if (!ERC20(_tokenContract).transferFrom(msg.sender, address(this), _amount))
            revert("transferFrom sender fallo");
    ...
    * @return bool true on success
     */
    function withdraw(bytes32 _contractId, bytes32 _preimage,string memory _firma)
        external
        contractExists(_contractId)
        hashlockMatches(_contractId, _firma)
    ...
    function calcular(string memory _input)
        public
        pure
        returns (bytes32)
    {
    ...
    function checkHash(bytes32 _contractId, string memory _x)external view{
        require(
            contracts[_contractId].hashlock == calcular(_x),
            "hashlock hash does not match"
            );
    ...
    function aprovarGasto(address _tokenContract,uint256 _amount)public{
        ERC20(_tokenContract).approve(address(this),_amount);
    }
